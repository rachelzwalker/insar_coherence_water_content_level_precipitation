...
class MyTestCase(unittest.TestCase):
    def test_something(self):
        DataRelationships(rolling_means_one_month, one_month_rolling_precipitation, max_lag=-50,
                          save_file_path=path_to_save_directory, site_name='Munsary EF',
                          time_scale='1_month').get_relationships()
        self.assertTrue(
            f'{path_to_save_directory}/cross_correlation_coherence_precipitation_1_month_Munsary EF_cross_corr.png')

    def test_get_lags(self):
        lags = DataRelationships(rolling_means_one_month, one_month_rolling_precipitation, max_lag=-50,
                          save_file_path=path_to_save_directory, site_name='Munsary EF',
                          time_scale='1_month').get_relationships()
        lags_df = lags['lags']
        lag_at_peak = lags_df.loc[lags_df['cross_correlation_time'].idxmax(), 'lags']
        self.assertTrue(lag_at_peak == 0)
